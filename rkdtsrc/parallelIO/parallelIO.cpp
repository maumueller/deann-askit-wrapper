                 ...
                 vector< pair<double, long> > *kNN)
{
    ...
    long buff_size = n * k; 
    unsigned int* indices= new unsigned int[buff_size];
    float* distances = new float[buff_size];
    cout << "n=" << n << ", k=" << K << endl;
    //cout<<"BigK = "<<BigK<<", n = "<<n<<", buff size = "<<buff_size*(long)sizeof(double)/1e9<<endl;

    long byte_offset = 2 * sizeof(int);
    infile.seekg(byte_offset, ifstream::beg);
    infile.read( (char*)(&(indices[0])), buff_size*sizeof(int) );
    infile.read( (char*)(&(distances[0])), buff_size*sizeof(int) );
    infile.close();
...
//        }
        for(long j = 0; j < k; j++) {
            (*kNN)[i*(long)k+j].first = distances[i * k + j];//buff[idx+1+2*j+0];
            (*kNN)[i*(long)k+j].second = indices[i * k + j]; //buff[idx+1+2*j+1];
        }
    }
