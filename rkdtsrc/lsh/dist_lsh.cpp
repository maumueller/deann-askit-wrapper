  ...
  for(int i = 0; i < sampleSize; i++) {
    ...
    for(int j = 0; j < mLocal; j++) {
      if( queryIDs[j] == (*sampleIDs)[i] ){
        #pragma omp critical
        {
          found.push_back( make_pair(queryIDs[j], j) );
        }
      }
    }
  }
...
pair<long,int> differenceFromTarget(double* localRef, int nLocal, long nGlobal, double *localQuery, 
  ...
  return std::make_pair(diffFromTarget, discards);
             ...
             MPI_Comm *comm_out)
{
  ...
  for( int i = 0; i < currRank; i++ ) process_queue.push( make_pair(0L, i) );
...
void knn::lsh::distPartitionedKQuery
  ...
  while( curriter < iters && (relative_error != relative_error /*NaN check*/ || relative_error > targetError ||
    ...
    bucketOffsets.push_back(make_pair(refKeys[0], 0));
    unsigned int currKey = refKeys[0];
    for(int i = 1; i < nLocal; i++) {
      if(refKeys[i] != currKey) {
        bucketOffsets.push_back(make_pair(refKeys[i], i));
        currKey = refKeys[i];
      }
    }
    bucketOffsets.push_back(make_pair(0xffffffff, nLocal)); //To find end of last bucket
    delete[] refKeys;


    vector<pair<unsigned int,int> > queryOffsets;
    queryOffsets.reserve(2*numBuckets/size);
    queryOffsets.push_back(make_pair(queryKeys[0], 0));
    currKey = queryKeys[0];
    for(int i = 1; i < mLocal; i++) {
      if(queryKeys[i] != currKey) {
        queryOffsets.push_back(make_pair(queryKeys[i], i));
        currKey = queryKeys[i];
      }
    }
    queryOffsets.push_back(make_pair(0xffffffff, mLocal)); //To find end of last bucket
    delete[] queryKeys;
    #if TIMINGVERBOSE
    MPI_Barrier(comm);
